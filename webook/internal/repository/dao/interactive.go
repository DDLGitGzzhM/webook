...
// biz = a bizId = 2
// biz = a bizId = 2
func (dao *GORMInteractiveDAO) BatchIncrReadCnt(
	ctx context.Context, bizs []string, ids []int64,
) error {
	// 可以用 map 合并吗？
...
func (dao *GORMInteractiveDAO) GetLikeInfo(
...
func (dao *GORMInteractiveDAO) GetCollectionInfo(
	ctx context.Context, biz string, bizId, uid int64,
...
func (dao *GORMInteractiveDAO) InsertCollectionBiz(
...
func (dao *GORMInteractiveDAO) InsertLikeInfo(
	ctx context.Context, biz string, bizId, uid int64,
) error {
	// 一把梭：同时记录点赞，以及更新点赞计数
	// 首先需要一张表来记录，谁给什么资源点了赞
	now := time.Now().UnixMilli()
	return dao.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		// 先准备插入点赞记录
		// 有没有可能已经点赞过了？
...
func (dao *GORMInteractiveDAO) DeleteLikeInfo(
	ctx context.Context, biz string, bizId, uid int64,
) error {
	now := time.Now().UnixMilli()
	// 控制事务超时
...
func (dao *GORMInteractiveDAO) IncrReadCnt(
...
func (dao *GORMInteractiveDAO) Get(
	ctx context.Context, biz string, bizId int64,
) (Interactive, error) {
	var res Interactive
	err := dao.db.WithContext(ctx).
...
// Interactive 正常来说，一张主表和与它有关联关系的表会共用一个 DAO，
// 所以我们就用一个 DAO 来操作
// 假如说我要查找点赞数量前 100 的，
// SELECT * FROM
...
type Interactive struct {
	...
	// 我这里 biz 用的是 string，有些公司枚举使用的是 int 类型
	// 0-article
	// 1- xxx
	// 默认是 BLOB/TEXT 类型
